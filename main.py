...
if __name__ == '__main__':
    ...
    print("################## TRAINING SUBJECT {} ##################".format(config["train"]["subject"]))

    if config["transform"]["name"] == "nBand":    
        #filterTransform = filterBank([[4,8],[8,12],[12,16],[16,20],[20,24],[24,28],[28,32],[32,36],[36,40]], 160)     ######################################
        #filterTransform = filterBank([[8,12],[12,16],[16,20],[20,24],[24,28],[28,32]], 160)
        #filterTransform = filterBank([[8,12]], 160)
        filterTransform = filterBank([[12,16],[16,20],[20,24],[24,28],[28,32]], 160) # Beta
    ...
    # save channels
    with open("../trained/"+"10/"+str(config["train"]["subject"])+'.txt', 'w') as filehandle: ##### Added the 10 for internship testing purposes
        ...
        for t in range(config["train"]["epochs"]):
            ...
            if (t > 50):
                ...
                if t > 250:
                    ...
                    torch.save(model.state_dict(), "../trained/"+"10c/"+str(config["train"]["subject"])+"_"+str(intra_acc)+"_"+str(inter_acc)+"_20"+".pth") ######## change here the name of the folder where models are saved
        print("Done!")
